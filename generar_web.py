...
:root{{--bg:#0a0c12;--surface:#13161f;--surface2:#1a1d27;--border:#252836;--accent:#b5841a;--text:#e2e8f0;--muted:#64748b;}}
...
<header>
  <div>
    <h1>☕ HAVANNA PRICE TRACKER</h1>
    <div class="tagline">Tienda Havanna — Argentina</div>
  </div>
...
<footer>Datos de tienda.havanna.com.ar · HAVANNABOT · Actualización diaria via GitHub Actions</footer>
<script>
const G = {graficos_js};
const RANK = {{dia:{rank_dia_js},d7:{rank_7d_js},mes:{rank_mes_js}}};
const CAT_COLORS = {{"Alfajores":"#b5841a","Havannets":"#d4a855","Chocolates":"#7c4f2a","Turrones":"#c8923a","Cajas y Packs":"#a06820","Otros":"#6366f1"}};
...
function renderMain(p){{
  ...
  chartMain=new Chart(document.getElementById('chartMain').getContext('2d'),{{
    type:'line',
    data:{{labels:d.map(x=>x.fecha),datasets:[{{label:'Total',data:d.map(x=>x.pct),borderColor:'#b5841a',backgroundColor:'rgba(181,132,26,0.06)',borderWidth:2,pointRadius:d.length>60?0:2,tension:0.3,fill:true}}]}},
...
renderMain('7d'); renderCats('7d'); mostrarRanking('dia');
    ...
    with open(DIR_DOCS / "index.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("Web HAVANNABOT generada: docs/index.html")
